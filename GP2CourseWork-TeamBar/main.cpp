...
//Camera Variables
float m_ForwardSpeed = 200.0f;
float m_StrafeSpeed = 200.0f;
...
int main(int argc, char * arg[])
{
	...
	while (running)
	{
		...
		if (Input::getInput().getKeyboard()->isKeyUp(SDLK_w))
		{
			forward = false;
		}
		if (Input::getInput().getKeyboard()->isKeyDown(SDLK_s))
		{
			backwards = true;
		}
		if (Input::getInput().getKeyboard()->isKeyUp(SDLK_s))
		{
			backwards = false;
		}
		...
		if (Input::getInput().getKeyboard()->isKeyUp(SDLK_a))
		{
			left = false;
		}
		if (Input::getInput().getKeyboard()->isKeyDown(SDLK_d))
		{
			rightM = true;
		}
		if (Input::getInput().getKeyboard()->isKeyUp(SDLK_d))
		{
			rightM = false;
		}
		if (backwards)
